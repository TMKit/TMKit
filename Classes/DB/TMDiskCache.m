...
static NSString * const TMDiskCacheOperationIdentifierTrimToSize = @"TMDiskCacheOperationIdentifierTrimToSize";
static NSString * const TMDiskCacheOperationIdentifierTrimToSizeByDate = @"TMDiskCacheOperationIdentifierTrimToSizeByDate";

typedef NS_ENUM(NSUInteger, TMDiskCacheCondition) {
    TMDiskCacheConditionNotReady = 0,
    TMDiskCacheConditionReady = 1,
};

static TMOperationDataCoalescingBlock TMDiskTrimmingSizeCoalescingBlock = ^id(NSNumber *existingSize, NSNumber *newSize) {
    NSComparisonResult result = [existingSize compare:newSize];
    return (result == NSOrderedDescending) ? newSize : existingSize;
};

static TMOperationDataCoalescingBlock TMDiskTrimmingDateCoalescingBlock = ^id(NSDate *existingDate, NSDate *newDate) {
    NSComparisonResult result = [existingDate compare:newDate];
    return (result == NSOrderedDescending) ? newDate : existingDate;
};

@interface TMDiskCacheMetadata : NSObject
@property (nonatomic, strong) NSDate *date;
@property (nonatomic, strong) NSNumber *size;
@end
@implementation TMDiskCacheMetadata
@end

@interface TMDiskCache () {
    TMDiskCacheSerializerBlock _serializer;
    TMDiskCacheDeserializerBlock _deserializer;
    
    TMDiskCacheKeyEncoderBlock _keyEncoder;
    TMDiskCacheKeyDecoderBlock _keyDecoder;
}
...
@property (strong, nonatomic) TMOperationQueue *operationQueue;
@property (strong, nonatomic) NSMutableDictionary <NSString *, TMDiskCacheMetadata *> *metadata;
...
- (instancetype)initWithName:(NSString *)name
                  ...
                  serializer:(TMDiskCacheSerializerBlock)serializer
                deserializer:(TMDiskCacheDeserializerBlock)deserializer
                  keyEncoder:(TMDiskCacheKeyEncoderBlock)keyEncoder
                  keyDecoder:(TMDiskCacheKeyDecoderBlock)keyDecoder
              operationQueue:(TMOperationQueue *)operationQueue
{
    if (!name)
        return nil;
...
+ (TMDiskCache *)sharedCache
{
    static TMDiskCache *cache;
    static dispatch_once_t predicate;
    
    dispatch_once(&predicate, ^{
        cache = [[TMDiskCache alloc] initWithName:TMDiskCacheSharedName];
    });
...
- (TMDiskCacheSerializerBlock)defaultSerializer
{
...
- (TMDiskCacheDeserializerBlock)defaultDeserializer
{
...
- (TMDiskCacheKeyEncoderBlock)defaultKeyEncoder
{
...
- (TMDiskCacheKeyEncoderBlock)defaultKeyDecoder
{
...
+ (BOOL)moveItemAtURLToTrash:(NSURL *)itemURL
{
    ...
    NSError *error = nil;
    NSString *uniqueString = [[NSProcessInfo processInfo] globallyUniqueString];
    NSURL *uniqueTrashURL = [[TMDiskCache sharedTrashURL] URLByAppendingPathComponent:uniqueString isDirectory:NO];
...
+ (void)emptyTrash
{
    dispatch_async([TMDiskCache sharedTrashQueue], ^{
        NSURL *trashURL = nil;
        ...
        if (trashURL != nil) {
            ...
            TMDiskCacheError(removeTrashedItemError);
        }
    });
}
...
- (void)initializeDiskProperties
{
    ...
    for (NSURL *fileURL in files) {
        ...
        if (_metadata[key] == nil) {
            _metadata[key] = [[TMDiskCacheMetadata alloc] init];
        }
...
- (BOOL)removeFileAndExecuteBlocksForKey:(NSString *)key
{
    ...
    TMCacheObjectBlock willRemoveObjectBlock = _willRemoveObjectBlock;
    ...
    TMCacheObjectBlock didRemoveObjectBlock = _didRemoveObjectBlock;
...
- (void)trimDiskToSize:(NSUInteger)trimByteCount
{
    ...
    if (_byteCount > trimByteCount) {
        NSArray *keysSortedBySize = [_metadata keysSortedByValueUsingComparator:^NSComparisonResult(TMDiskCacheMetadata * _Nonnull obj1, TMDiskCacheMetadata * _Nonnull obj2) {
...
- (void)trimDiskToSizeByDate:(NSUInteger)trimByteCount
{
    [self lockForWriting];
    if (_byteCount > trimByteCount) {
        NSArray *keysSortedByDate = [_metadata keysSortedByValueUsingComparator:^NSComparisonResult(TMDiskCacheMetadata * _Nonnull obj1, TMDiskCacheMetadata * _Nonnull obj2) {
...
- (void)lockFileAccessWhileExecutingBlockAsync:(TMCacheBlock)block
{
    if (block == nil) {
        return;
    }
...
- (void)containsObjectForKeyAsync:(NSString *)key completion:(TMDiskCacheContainsBlock)block
{
    if (!key || !block)
        return;
...
- (void)objectForKeyAsync:(NSString *)key completion:(TMDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        NSURL *fileURL = nil;
...
- (void)fileURLForKeyAsync:(NSString *)key completion:(TMDiskCacheFileURLBlock)block
{
    if (block == nil) {
        return;
    }
...
- (void)setObjectAsync:(id <NSCoding>)object forKey:(NSString *)key completion:(TMDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        NSURL *fileURL = nil;
...
- (void)setObjectAsync:(id <NSCoding>)object forKey:(NSString *)key withCost:(NSUInteger)cost completion:(nullable TMCacheObjectBlock)block
{
    [self setObjectAsync:object forKey:key completion:(TMDiskCacheObjectBlock)block];
}

- (void)removeObjectForKeyAsync:(NSString *)key completion:(TMDiskCacheObjectBlock)block
{
    [self.operationQueue scheduleOperation:^{
        NSURL *fileURL = nil;
...
- (void)trimToSizeAsync:(NSUInteger)trimByteCount completion:(TMCacheBlock)block
{
    TMOperationBlock operation = ^(id data) {
    ...
    [self.operationQueue scheduleOperation:operation
                              withPriority:TMOperationQueuePriorityLow
                                identifier:TMDiskCacheOperationIdentifierTrimToSize
                            coalescingData:[NSNumber numberWithUnsignedInteger:trimByteCount]
                       dataCoalescingBlock:TMDiskTrimmingSizeCoalescingBlock
                                completion:completion];
}

- (void)trimToDateAsync:(NSDate *)trimDate completion:(TMCacheBlock)block
{
    TMOperationBlock operation = ^(id data){
    ...
    [self.operationQueue scheduleOperation:operation
                              withPriority:TMOperationQueuePriorityLow
                                identifier:TMDiskCacheOperationIdentifierTrimToDate
                            coalescingData:trimDate
                       dataCoalescingBlock:TMDiskTrimmingDateCoalescingBlock
                                completion:completion];
}

- (void)trimToSizeByDateAsync:(NSUInteger)trimByteCount completion:(TMCacheBlock)block
{
    TMOperationBlock operation = ^(id data){
    ...
    [self.operationQueue scheduleOperation:operation
                              withPriority:TMOperationQueuePriorityLow
                                identifier:TMDiskCacheOperationIdentifierTrimToSizeByDate
                            coalescingData:[NSNumber numberWithUnsignedInteger:trimByteCount]
                       dataCoalescingBlock:TMDiskTrimmingSizeCoalescingBlock
                                completion:completion];
}

- (void)removeAllObjectsAsync:(TMCacheBlock)block
{
    [self.operationQueue scheduleOperation:^{
...
- (void)enumerateObjectsWithBlockAsync:(TMDiskCacheFileURLEnumerationBlock)block completionBlock:(TMCacheBlock)completionBlock
{
    [self.operationQueue scheduleOperation:^{
...
- (void)synchronouslyLockFileAccessWhileExecutingBlock:(TMCacheBlock)block
{
    if (block) {
        [self lockForWriting];
        block(self);
    ...
    if (!self->_ttlCache || self->_ageLimit <= 0 || fabs([_metadata[key].date timeIntervalSinceDate:now]) < self->_ageLimit) {
        ...
        if (objectData) {
            ...
            @catch (NSException *exception) {
                ...
                [self unlock];
                TMDiskCacheError(error)
                TMDiskCacheException(exception);
            }
            [self lock];
        }
...
- (void)setObject:(id <NSCoding>)object forKey:(NSString *)key fileURL:(NSURL **)outFileURL
{
    ...
    [self lockForWriting];
    TMCacheObjectBlock willAddObjectBlock = self->_willAddObjectBlock;
    ...
    TMCacheObjectBlock didAddObjectBlock = self->_didAddObjectBlock;
...
- (void)removeAllObjects
{
    ...
    [self lockForWriting];
    TMCacheBlock willRemoveAllObjectsBlock = self->_willRemoveAllObjectsBlock;
    ...
    TMCacheBlock didRemoveAllObjectsBlock = self->_didRemoveAllObjectsBlock;
...
- (void)enumerateObjectsWithBlock:(TMDiskCacheFileURLEnumerationBlock)block
{
    if (!block)
        return;
...
- (TMCacheBlock)willRemoveAllObjectsBlock
{
    TMCacheBlock block = nil;
...
- (void)setWillRemoveAllObjectsBlock:(TMCacheBlock)block
{
    [self.operationQueue scheduleOperation:^{
        [self lock];
...
- (TMCacheBlock)didRemoveAllObjectsBlock
{
    TMCacheBlock block = nil;
...
- (void)setDidRemoveAllObjectsBlock:(TMCacheBlock)block
{
    [self.operationQueue scheduleOperation:^{
        [self lock];
